...
class FormService
{
    ...
    public function validateBotzappingAuth($api_key, $tag_hash_key, $secret_key, $action)
    {
        LogService::logErrorCode(ErrorCodes::PLUGIN_START_AUTHENTICATE);
        if ($api_key && $tag_hash_key && $secret_key) {
            $rtiService = new RTI_Service();
            $clientId = $rtiService->auth_with_botzapping($api_key, $tag_hash_key, $secret_key, $action);
